...
class ResultView extends GetView<ResultController> {
  ...
  Widget _buildDetailsCard() {
    final data = controller.diplomaData.value;
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          children: [
            _buildDetailItem(Icons.person_outline, 'Titulaire', '${data['studentName'] ?? 'N/A'}'),
            _buildDetailItem(Icons.school_outlined, 'Formation', data['title'] ?? 'N/A'),
            _buildDetailItem(Icons.account_balance_outlined, 'Établissement', data['institutionName'] ?? 'N/A'),
            _buildDetailItem(Icons.star_outline, 'Mention', data['grade'] ?? 'N/A'),
            _buildDetailItem(Icons.calendar_today_outlined, 'Année académique', data['issueDate'] != null ? data['issueDate'].toString().substring(0, 10) : 'N/A'),
            _buildDetailItem(Icons.numbers_outlined, 'Numéro de diplôme', data['certificateNumber'] ?? 'N/A'),
          ],
        ),
      ),
    );
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Informations Blockchain', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14.sp)),
            SizedBox(height: 16.h),
            _buildDetailItem(Icons.link, 'Statut Blockchain', 'Certifié et Immuable'),
            _buildDetailItem(Icons.verified_user_outlined, 'Signataire', data['institutionSigle'] ?? data['institutionName'] ?? 'N/A'),
            _buildDetailItem(Icons.fingerprint, 'Transaction Hash', (data['blockchainHash'] as String?)?.substring(0, 16) ?? 'Vérifié par Smart Contract'),
            SizedBox(height: 16.h),
            Container(
