...
class SettingsTab extends GetView<DashboardController> {
  ...
  Widget _buildOrganizationProfile() {
    return Obx(() {
      ...
      final fullName = user.isNotEmpty ? '${user['firstName']} ${user['lastName']}' : 'Utilisateur';
      final email = user.isNotEmpty ? user['email'] : 'email@exemple.com';
      final role = user.isNotEmpty ? user['role'].toString().toUpperCase() : 'MEMBRE';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  CircleAvatar(
                    ...
                    child: const Icon(Icons.person, color: AppColors.darkBlue, size: 30),
                  ),
                  SizedBox(width: 16.w),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(fullName, style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.bold)),
                        Text(email, style: TextStyle(fontSize: 12.sp, color: AppColors.textGrey)),
                      ],
                    ),
                  ),
                  IconButton(
              ...
              _buildInfoRow('Statut du compte', user['isVerified'] == true ? 'Vérifié' : 'Actif', isPremium: user['isVerified'] == true),
              _buildInfoRow('Identifiant', user.isNotEmpty ? user['_id'].toString().substring(0, 8) : 'N/A'),
              SizedBox(height: 24.h),
              SizedBox(
  ...
  Widget _buildTeamSection() {
    ...
    if (user != null && user['role'] == 'student') return const SizedBox.shrink();
